// Package official mongo driver, eases the computation of pagination information
// of a find mongo query by augmenting the base query with cursor information and
// returning a cursor.
...
func Find(p FindParams, results interface{}) (Cursor, error) {
	...
	hasMore := int64(resultsVal.Len()) > p.Limit
...
// decodeCursor decodes cursor data that was previously encoded with encodeCursor
...
var executeCountQuery = func(db *mongo.Database, collectionName string, queries []bson.M) (int64, error) {
	return db.Collection(collectionName).CountDocuments(context.Background(), bson.M{"$and": queries})
}
...
var executeCursorQuery = func(db *mongo.Database, collectionName string, query []bson.M, sort []string, limit int64, collation *options.Collation, results interface{}) error {
	ctx := context.Background()
	plimit := limit + 1
	fmt.Sprintf("%s", plimit)
	...
	cursor, err := db.Collection(collectionName).Find(ctx, bson.M{"$and": query}, &findOpts)
	if err != nil {
		return err
	}
...
func generateCursor(result interface{}, paginatedField string, shouldSecondarySortOnID bool) (string, error) {
	...
	cursorData = append(cursorData, primitive.E{Key: paginatedField, Value: paginatedFieldValue})
	if shouldSecondarySortOnID {
		...
		cursorData = append(cursorData, primitive.E{Key: "_id", Value: id})
	}
